...
describe('LogsPage evidence view', () => {
  ...
  beforeEach(() => {
    ...
    vi.stubGlobal('fetch', vi.fn((url: string) => {
      if (url === '/api/dashboard/summary') return jsonResponse({ kpis: { scheduled_posts: 2, published_posts: 1, total_channels: 1, total_reach: 0 }, status_bar: { buffer_api: 'not_configured', imgbb_api: 'not_configured', pancake: 'not_synced' }, scheduled_targets: 2, published_targets: 1, failed_targets: 1, total_channels: 1 })
      ...
      if (url === '/api/audit-logs?limit=12') return jsonResponse({ items: [{ id: 'audit-1', created_at: '2026-05-19T09:01:00Z', actor_user_id: 'user-a', actor_agent_id: null, action: 'publish_job.created', resource_type: 'publish_job', resource_id: 'job-12345678', data: { dry_run: true, result: 100004822807900, access_token: 'secret-token', cookie: 'credential-cookie', profile_url: 'https://facebook.com/profile.php?id=100004822807900', fb_uid: '100004822807900', authorization: 'Bearer abcdefghijk' } }], limit: 12 })
  ...
  it('renders browser-safe demo evidence from Cloud APIs', async () => {
    ...
    expect(screen.getByText('Gate A+B verification')).toBeTruthy()
    expect(screen.getByText(/zoopost-verify-all\.ps1 -Python \$python -IncludeSafetyGate/)).toBeTruthy()
    expect(screen.getByText(/Dry-run pilot only/)).toBeTruthy()
    ...
    await waitFor(() => {
      ...
      expect(screen.queryByText(/Bearer abcdefghijk/i)).toBeNull()
    })
    expect(screen.getByText(/dry_run=true, redacted_fields=6/)).toBeTruthy()
  })
