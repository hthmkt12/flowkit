...
import type { SocialChannel, ContentPreviewResult, PublishJob } from '../types'
...
export default function AutoPostFanpagePage() {
  ...
  const [body, setBody] = useState('Xin chào [r]\n$SPIN=[Nội dung A | Nội dung B | Nội dung C]')
  const [title, setTitle] = useState('ZooPost dry-run')
  ...
  const normalizedMinDelay = Number.isFinite(minDelay) ? minDelay : 3
  const normalizedMaxDelay = Number.isFinite(maxDelay) ? maxDelay : 10
  const safeMinDelay = Math.max(3, Math.min(normalizedMinDelay, normalizedMaxDelay))
  ...
  async function renderPreview() {
    try {
      const result = await postAPI<ContentPreviewResult>('/api/content-items/preview', { body, seed: selected[0] ?? 'preview' })
      setPreview(result)
      setMessage(null)
    } catch (error) {
      setPreview(null)
  ...
  async function startDryRunJob() {
    ...
    setJob(null)
    try {
      const content = await postAPI<{ id: string }>('/api/content-items', { title, body, syntax_mode: 'zoopost', status: 'draft' })
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(280px, 1.1fr) minmax(260px, 0.8fr) minmax(300px, 0.9fr)', gap: '14px', alignItems: 'start' }}>
        <section style={panelStyle()}>
          ...
          <input value={title} onChange={event => setTitle(event.target.value)} placeholder="Tiêu đề nội dung" style={inputStyle()} />
          <textarea value={body} onChange={event => setBody(event.target.value)} rows={12} style={{ ...inputStyle(), resize: 'vertical', lineHeight: 1.5 }} />
        ...
        <section style={panelStyle()}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
            <input type="number" min={3} value={minDelay} onChange={event => setMinDelay(parseDelayInput(event.target.value, 3))} style={inputStyle()} />
            <input type="number" min={3} value={maxDelay} onChange={event => setMaxDelay(parseDelayInput(event.target.value, 10))} style={inputStyle()} />
          </div>
      ...
      {job && <ProgressPreview job={job} />}
    </div>
  )
}

function ProgressPreview({ job }: { job: PublishJob }) {
  const total = job.targets.length
  const queued = job.targets.filter(target => ['queued', 'retry'].includes(target.status)).length
  ...
  const percent = total === 0 ? 0 : Math.round((posted + failed) * 100 / total)
  return (
    <section style={panelStyle()}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '180px 1fr', gap: '16px' }}>
        <div style={{ display: 'grid', placeItems: 'center', width: '150px', height: '150px', borderRadius: '50%', background: `conic-gradient(var(--green) 0 ${percent}%, var(--border) ${percent}% 100%)` }}>
          <div style={{ display: 'grid', placeItems: 'center', width: '104px', height: '104px', borderRadius: '50%', background: 'var(--card)' }}>
            ...
            <div style={{ fontSize: '10px', color: 'var(--muted)' }}>Dry-run</div>
          </div>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          ...
          <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
            ...
            <span style={chipStyle()}><Clock size={12} /> Đang chờ {queued}</span>
