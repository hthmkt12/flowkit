...
**Goal:** Add `scripts/start-fbkit-safe.ps1` so Windows users can start `agent.main` with safe FBKit env defaults, or verify command output via `-NoStart` / `-PrintOnly` without launching server/browser/Facebook.

## Phase 1 — Script contract + tests

- **Files:** create `scripts/start-fbkit-safe.ps1`; create `tests/unit/test_start_fbkit_safe_script.py`.
...
- **Test matrix:** static-read script text; run PowerShell with `-NoStart` or `-PrintOnly`; assert env defaults + python/module command; assert no `Start-Process`, browser launch, Facebook URL, HTTP calls, or server start in print mode.
- **Verification:** `& ".\.venv\Scripts\python.exe" -m pytest tests\unit\test_start_fbkit_safe_script.py -q`.
...
- **Verification:** same unit test plus static README assertion optional in `tests/unit/test_start_fbkit_safe_script.py`.

## Dependency graph / ownership

- Phase 1 before Phase 2, because README should document final flags.
- File ownership: script owner touches `scripts/start-fbkit-safe.ps1`; test owner touches `tests/unit/test_start_fbkit_safe_script.py`; docs owner touches `README.md`. No parallel overlap.

## Rollback

- Delete `scripts/start-fbkit-safe.ps1` and `tests/unit/test_start_fbkit_safe_script.py`; revert README helper paragraph. Existing manual startup remains working.
...
- Should both `-NoStart` and `-PrintOnly` be aliases, or only one canonical flag with alias?
